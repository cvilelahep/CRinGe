...
#            torch.nn.BatchNorm1d(3),
        ...
        self._upconvs = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(32, 1+N_GAUS*5, 3)                                  # 88 x 168
        )
...
#        self._softmax = torch.nn.Softmax(dim=1)
    ...
    def forward(self, x) :
        ...
        net = self._upconvs(net).view(-1, 1+N_GAUS*5, 88*168)
...
#            loss = blob.criterion(prediction, label)
            ...
            timeloss = (1-fracUnhit)*(1/2.)*np.log(2*np.pi)-(1-fracUnhit)*torch.logsumexp(-1/2.*logtvar[:,~unhitMask] - 1/2. *(time_n[:,~unhitMask]-tmu[:,~unhitMask])**2/tvar[:,~unhitMask], dim = 0).mean()   
