...
#            loss += (1-fracUnhit)*(1/2.)*np.log(2*np.pi)
            ...
            loss += (1-fracUnhit)*torch.log(label[~unhitMask]*torch.sqrt(2*np.pi*var[~unhitMask])).mean()
            loss += (1-fracUnhit)*((torch.log(label[~unhitMask]) - mu[~unhitMask])**2/(2*var[~unhitMask]**2)).mean()
...
while epoch < TRAIN_EPOCH :
    ...
    for i,data in enumerate(train_loader) :
        ...
        if (iteration+1)%7363 == 0 :
            torch.save(blob.net.state_dict(), "testCRinGe_LogNormal_i_"+str(iteration)+".cnn")
        if epoch >= TRAIN_EPOCH :
            break

torch.save(blob.net.state_dict(), "testCRinGe_LogNormal.cnn")
