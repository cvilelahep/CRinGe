...
top_mask = file_handle['mask'][0].reshape(-1, dim_cap[0]*dim_cap[1])
bottom_mask = file_handle['mask'][1].reshape(-1, dim_cap[0]*dim_cap[1])
...
for data in [ [[1, 0, 0, 0., 0., 0., 0., 0., 1., 200]],
    ...
    eventMu = np.exp(net(data)[cap_type].cpu().detach().numpy()[0,1] * mask[cap_type-1] )
    ...
    eventVariance = np.exp(net(data)[cap_type].cpu().detach().numpy()[0,0] * mask[cap_type-1])
    ...
    eventHitProb = 1./(1+np.exp(net(data)[cap_type].cpu().detach().numpy()[0,2])) * mask[cap_type-1]
...
figMu.savefig("CRinGe_Gaus_SK_{Ncap}_Mu.png".format(Ncap=cap_name[cap_type-1]))
figVariance.savefig("CRinGe_Gaus_SK_{Ncap}_Variance.png".format(Ncap=cap_name[cap_type-1]))
figHitProb.savefig("CRinGe_Gaus_SK_{Ncap}_HitProb.png".format(Ncap=cap_name[cap_type-1]))
figMuHitProb.savefig("CRinGe_Gaus_SK_{Ncap}_MuHitProb.png".format(Ncap=cap_name[cap_type-1]))
