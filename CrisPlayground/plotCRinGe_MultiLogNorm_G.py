...
print("PLOTTING NN from "+str(N_GAUS)+" LogNorms in LOSS")
...
#            torch.nn.BatchNorm1d(3),
        ...
        self._upconvs = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(32, 1+N_GAUS*4, 3)
        )
    ...
    def forward(self, x) :
        ...
        net = self._upconvs(net).view(-1, 1+N_GAUS*4, 88*168)
        # return net
...
def LogNorm(nG, x, coeff, mu, var, shift):
    lognorm = [0]*(nG+1)
    #coefftot = 0
    tot = 0
    if nG != len(mu):
        print('LogNorm input dimensions do not agree!')
        return lognorm
    for i in range(nG):
        tot += coeff[i]*(1./(2*np.pi*(x+shift[i])*var[i])**0.5)*np.exp(-np.power(np.log(x+shift[i]) - mu[i], 2.) / (2 * var[i]))
        lognorm[i] = (coeff[i]*(1./(2*(x+shift[i])*np.pi*var[i])**0.5)*np.exp(-np.power(np.log(x+shift[i]) - mu[i], 2.) / (2 * var[i])))
        #coefftot += coeff[i]
    #return  gauss / coefftot
    lognorm[-1] = tot
    return  lognorm

def Tpeak(x, mu, var, shift):
    tpeak = 0
    tpeak = ((1./(2*(x+50+shift)*np.pi*var)**0.5)*np.exp(-(np.log(x+50+shift)-mu)**2 / (2 * var)))
    return tpeak
...
net = CRinGeNet().cpu()
net.load_state_dict(torch.load("testCRinGe_MultiLogNorm_"+str(N_GAUS)+".cnn", map_location=lambda storage, loc: storage))

torch.set_grad_enabled(False)
net.eval()

#fIn = h5py.File('/home/junjiex/projects/def-pdeperio/junjiex/HKML/varyAll/IWCDgrid_varyAll_mu-_20-2000MeV_1M.h5','r')

#event_data = fIn['event_data']
#print(event_data.shape)
#energy_data = fIn['energies']
#plot_event = event_data[300]

#nEvent = len(event_data)
nEvent = 1
...
shiftArr = [[] for i in range (N_GAUS)]
...
for data in [ [[1, 0, 0, 0., 0., 0., 0., 0., 1., 200]],
    ...
    for iax in range(N_GAUS):            
        ...
        eventShift = np.exp(net(data).cpu().detach().numpy()[0,3+iax*3])
        eventShift = eventShift.reshape((88,168))
        shiftArr[iax].append(eventShift.flatten())

        eventMu = np.exp(net(data).cpu().detach().numpy()[0,2+iax*3])
        eventMu = eventMu.reshape((88,168))
        muArr[iax].append(eventMu.flatten())
    
        eventVariance = np.exp(net(data).cpu().detach().numpy()[0,1+iax*3])
...
#print(muArr)
#print(len(hitProbArr),len(hitProbArr[0]))

#print(len(muArr), len(muArr[0]))
#print(len(muArr), len(coeffArr[0]))
#maxIndex = np.where(hitProbArr[0] == np.amax(hitProbArr[0]))
#maxIndex = 7787
...
for i in range(len(muArr[0])):
    ...
    for j in range(len(muArr)):
        ...
        shifttemp.append(shiftArr[j][i][maxIndex])
        coefftemp.append(coeffArr[j][i][maxIndex])

#        mutemp.append(muArr[j][i][maxIndex][0])
#        vartemp.append(varArr[j][i][maxIndex][0])
#        coefftemp.append(coeffArr[j][i][maxIndex][0])
#        print(i, j, mutemp[j], vartemp[j], coefftemp[j])
    coefftemp = user_softmax(coefftemp)
    lognorm = LogNorm(N_GAUS, x, coefftemp, mutemp, vartemp, shifttemp)
    for k in range(N_GAUS+1):
        lnmax = np.amax(lognorm[k])
        lnmaxid = np.where(lognorm[k] == lnmax)
        lnupMask = (lognorm[k] >= 0.5*lnmax)
        lnfwhm = np.array(lnupMask, dtype=int)
        if k < N_GAUS:
            thisAx.plot(x, lognorm[k], linestyle = ':')
        else:
            thisAx.plot(x, lognorm[k], color = 'black', linewidth = 2)

    for text in thisAx.texts :
        txt.set_visible(False)
        
    thisAx.text(0.8, 0.9, "peak(FWHM) = %.2f(%.2f)" % (x[lnmaxid], StepSize*np.sum(lnfwhm)), horizontalalignment = 'center', verticalalignment = 'center', transform=thisAx.transAxes)
...
figQDist.savefig("PNGplots/CRinGe_MultiLogNorm_"+str(N_GAUS)+"_PMT_"+str(maxIndex)+"_LogNormFitwShift.png")
