...
net.load_state_dict(torch.load("testCRinGe_LogNormal.cnn", map_location=lambda storage, loc: storage))
...
#    dim = data[0].shape
    ...
    predMean = np.exp(predMu+predVar/2)
    predVariance = (np.exp(predVar) - np.ones(predVar.shape))*np.exp(2*predMu+predVar)
    
    thrownEvent = np.random.uniform(size = predHitProb.shape)
    thrownEvent = (thrownEvent > predHitProb).astype(float)
    thrownEvent *= np.exp(np.multiply((predVar**0.5), np.random.normal(size = predHitProb.shape)) + predMu)
    ...
    while np.sum(negatives) != 0 :
        ...
        thrownEvent[negatives] *= np.exp(np.multiply((predVar[negatives]**0.5), np.random.normal(size = predHitProb.shape)[negatives]) + predMu[negatives])
        negatives = thrownEvent < 0
    ...
    if i == N_Plot_Residual :
        ...
        plt.subplot(2, 2, 1)
        plt.imshow(predMean)
        plt.title("NN mean charge prediction")
        plt.colorbar()
        plt.subplot(2, 2, 3)
        ...
        plt.colorbar()
        plt.subplot(2, 2, 2)
        plt.imshow(predMean-event_q)
        plt.title("Residual")
        plt.colorbar()
        plt.tight_layout()
        plt.savefig("compare_event_prediction_LogNormal_"+str(i)+".png")

        plt.figure()
        thisQMask = (event_q > 1.5) & (event_q < 2.0)
        thisQMaskPos = thisQMask & ((event_q - predMean)/np.sqrt(predVariance) > 0)
        thisQMaskNeg = thisQMask & ((event_q - predMean)/np.sqrt(predVariance) < 0)
    ...
    hitMask = event_q > 0
    thesePulls = np.hstack((event_q[hitMask] - predMean[hitMask])/np.sqrt(predVariance[hitMask]))
    charges = np.append(charges, np.hstack(event_q[hitMask]))
    mus = np.append(mus, np.hstack(predMean[hitMask]))
    ...
    theseThrownPulls = np.hstack((thrownEvent[thrownHitMask] - predMean[thrownHitMask])/np.sqrt(predVariance[thrownHitMask]))
