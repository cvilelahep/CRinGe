...
def forward(blob, train=True) :
    with torch.set_grad_enabled(train) :
        ...
        if blob.label_top is not None:
            ...
            CTloss += -(1-fracUnhit_top)*torch.logsumexp(torch.log(coefficients_top[:,~unhitMask_top]) + loga11_top[:,~unhitMask_top] + loga22_top[:,~unhitMask_top] - 1/2.*((tmu_sub_time_top[:,~unhitMask_top]*a11_top[:,~unhitMask_top])**2 + mu_sub_label_top[:,~unhitMask_top]**2*(a22_top[:,~unhitMask_top]**2 + a12_top[:, ~unhitMask_top]**2) + 2*tmu_sub_time_top[:,~unhitMask_top]*mu_sub_label_top[:,~unhitMask_top]*a11_top[:,~unhitMask_top]*a12_top[:,~unhitMask_top]), dim=0).mean()
        ...
        if blob.label_bottom is not None:
            ...
            CTloss += -(1-fracUnhit_bottom)*torch.logsumexp(torch.log(coefficients_bottom[:,~unhitMask_bottom]) + loga11_bottom[:,~unhitMask_bottom] + loga22_bottom[:,~unhitMask_bottom] - 1/2.*((tmu_sub_time_bottom[:,~unhitMask_bottom]*a11_bottom[:,~unhitMask_bottom])**2 + mu_sub_label_bottom[:,~unhitMask_bottom]**2*(a22_bottom[:,~unhitMask_bottom]**2 + a12_bottom[:, ~unhitMask_bottom]**2) + 2*tmu_sub_time_bottom[:,~unhitMask_bottom]*mu_sub_label_bottom[:,~unhitMask_bottom]*a11_bottom[:,~unhitMask_bottom]*a12_bottom[:,~unhitMask_bottom]), dim=0).mean()
...
def fillTime (blob, data) :
    dim = data[0].shape
    dim_cap = data[5].shape
    blob.time = (0.001*(data[0][:,:,:,1]-932.)).reshape(-1, dim[1]*dim[2])
    blob.time_top = (0.001*(data[5][:,:,:,1]-932.)).reshape(-1, dim_cap[1]*dim_cap[2])
    blob.time_bottom = (0.001*(data[6][:,:,:,1]-932.)).reshape(-1, dim_cap[1]*dim_cap[2])
...
while epoch < TRAIN_EPOCH :
    ...
    for i,data in enumerate(train_loader) :
        ...
        if (iteration+1)%7363 == 0 :
            ...
            torch.save(blob.net.state_dict(), "testCRinGe_MultiGausTimeCorr_SK_"+str(N_GAUS)+"_i_"+str(iteration)+".cnn")
        if epoch >= TRAIN_EPOCH :
            break

#torch.save(blob.net.state_dict(), "testCRinGe_MultiGausTimeCorr_"+str(N_GAUS)+".cnn")
torch.save(blob.net.state_dict(), "testCRinGe_MultiGausTimeCorr_SK_"+str(N_GAUS)+".cnn")
#sys.stdout.close
