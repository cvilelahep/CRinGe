...
net.load_state_dict(torch.load("testCRinGe_EMG.cnn", map_location=lambda storage, loc: storage))
...
#    dim = data[0].shape
    ...
    prediction = prediction.reshape((4,88,168))
    
    predMu = np.exp(prediction[1])
    predVar = np.exp(prediction[0])
    predLam = np.exp(prediction[2])
    predHitProb = 1/(1+np.exp(-1*prediction[3]))

    thrownEvent = np.random.uniform(size = predHitProb.shape)
    thrownEvent = (thrownEvent > predHitProb).astype(float)
    thrownEvent *= np.multiply((predVar**0.5), np.random.normal(size = predHitProb.shape)) + predMu + np.multiply(np.random.exponential(size = predHitProb.shape), 1./predLam)
    ...
    while np.sum(negatives) != 0 :
        ...
        thrownEvent[negatives] *= np.multiply((predVar[negatives]**0.5), np.random.normal(size = predHitProb.shape)[negatives]) + predMu[negatives] + np.multiply(np.random.exponential(size = predHitProb.shape)[negatives], 1./predLam[negatives])
        negatives = thrownEvent < 0
    ...
    if i == N_Plot_Residual :
        ...
        plt.subplot(2, 2, 1)
        plt.imshow(np.multiply((1.-predHitProb), predMu+1./predLam))
        ...
        plt.colorbar()
        plt.tight_layout()
        plt.savefig("compare_event_prediction_EMG_"+str(i)+".png")

        plt.figure()
        thisQMask = (event_q > 1.5) & (event_q < 2.0)
        thisQMaskPos = thisQMask & ((event_q - (predMu+1./predLam))/np.sqrt(predVar+1./np.square(predLam)) > 0)
        thisQMaskNeg = thisQMask & ((event_q - (predMu+1./predLam))/np.sqrt(predVar+1./np.square(predLam)) < 0)
    ...
    hitMask = event_q > 0
    thesePulls = np.hstack((event_q[hitMask] - (predMu[hitMask]+1./predLam[hitMask]))/np.sqrt(predVar[hitMask] + 1./np.square(predLam[hitMask]) ))
    charges = np.append(charges, np.hstack(event_q[hitMask]))
    mus = np.append(mus, np.hstack(predMu[hitMask]+1./predLam[hitMask]))
    lambdas = np.append(lambdas, np.hstack(predLam[hitMask]))
    ...
    theseThrownPulls = np.hstack((thrownEvent[thrownHitMask] - (predMu[thrownHitMask]+1./predLam[thrownHitMask]))/np.sqrt(predVar[thrownHitMask]+1./np.square(predLam[thrownHitMask])))
