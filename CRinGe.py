...
blob.distance = torch.nn.PairwiseDistance(p = 1) # Euclidean distance
blob.data = None
blob.label = None
...
DATA_DIRS=['/home/cvilela/HKML/varyAllBarOnly']
train_loader=loader_factory('H5Dataset', batch_size=64, shuffle=True, num_workers=2, data_dirs=DATA_DIRS, flavour='100k.h5', start_fraction=0.0, use_fraction=0.75, read_keys= ["positions","directions", "energies"])
test_loader=loader_factory('H5Dataset', batch_size=200, shuffle=True, num_workers=2, data_dirs=DATA_DIRS, flavour='100k.h5', start_fraction=0.75, use_fraction=0.25, read_keys= [ "positions","directions", "energies"])
...
def fillData (blob,data) :
    ...
    blob.data =  np.hstack((oneHotE.reshape(len(oneHotE),1), oneHotMu.reshape(len(oneHotMu),1), # One-hot PID
                            data[2][:,0,:], # Positions
                            data[3][:,0,:], # Directions
